...
parser.add_argument("--eval_batch_size", type=int, default=500, help="size of the batches")
...
parser.add_argument("--gen_sample_size", type=int, default=50000, help="Sample size for evaluation")
...
if opt.sample_size is not None:
    # randomly select a subset of the data
   data = data[np.random.choice(data.shape[0], opt.sample_size, replace=False), :]
...
# training_idx, test_idx = indices[:int(0.99 * sampleSize)], indices[int(0.01 * sampleSize):]
...
if opt.training:
    ...
    if opt.resume:
        ...
        checkpoint = torch.load(os.path.join(opt.expPATH, f"model_epoch_{opt.n_epochs}.pth"))
...
if opt.generate:
    ...
    num_fake_samples = opt.gen_sample_size

    # Generate a batch of samples
    gen_samples = np.zeros((num_fake_samples, real_samples.shape[1]), dtype=type(real_samples))
    n_batches = int(num_fake_samples / opt.eval_batch_size)
    # start = time.time()
    for i in range(n_batches):
        # Sample noise as generator input
        # z = Variable(Tensor(np.random.normal(0, 1, (opt.batch_size, opt.latent_dim))))
        z = torch.randn(opt.eval_batch_size, opt.latent_dim, device=device)
        ...
        gen_samples[i * opt.eval_batch_size:(i + 1) * opt.eval_batch_size, :] = gen_samples_decoded.cpu().data.numpy()
        # Check to see if there is any nan
        assert (gen_samples[i, :] != gen_samples[i, :]).any() == False

    gen_samples = np.delete(gen_samples, np.s_[(i + 1) * opt.eval_batch_size:], 0)
    ...
    # ave synthetic data
    np.save(os.path.join(opt.expPATH, f"synthetic-{opt.gen_sample_size}.npy"), gen_samples, allow_pickle=False)
