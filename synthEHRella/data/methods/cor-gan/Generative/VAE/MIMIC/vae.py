...
class VAE(nn.Module):
    ...
    def forward(self, x):
        mu, logvar = self.encode(x.view(-1, 2254))
...
def loss_function(recon_x, x, mu, logvar):
    BCE = F.binary_cross_entropy(recon_x, x.view(-1, 2254), reduction='sum')
...
def generate():
    ...
    n_batches = int(num_fake_samples / args.eval_batch_size)
    for i in range(n_batches):
        random_input = torch.randn(args.eval_batch_size, args.compress_dim).to(device)
        sample = model.decode(random_input).cpu().data.numpy()
        gen_samples[i * args.eval_batch_size:(i + 1) * args.eval_batch_size, :] = sample
        # Check to see if there is any nan
        assert (gen_samples[i, :] != gen_samples[i, :]).any() == False

    gen_samples = np.delete(gen_samples, np.s_[(i + 1) * args.eval_batch_size:], 0)
